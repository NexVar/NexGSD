...
"git": {
  ...
  "phase_branch_template": "nexgsd/phase-{phase}-{slug}",
  "milestone_branch_template": "nexgsd/{milestone}-{slug}"
}
```
...
| `git.phase_branch_template` | `"nexgsd/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"nexgsd/{milestone}-{slug}"` | Branch template for milestone strategy |
</config_schema>
...
**Note:** Most NexGSD operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
- Standard NexGSD behavior

**When `git.branching_strategy: "phase"`:**
- `execute-phase` creates/switches to a branch before execution
- Branch name from `phase_branch_template` (e.g., `nexgsd/phase-03-authentication`)
...
- Branch name from `milestone_branch_template` (e.g., `nexgsd/v1.0-mvp`)
