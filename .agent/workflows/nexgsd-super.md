...
```
/nexgsd-super Build a task management app with Next.js and Prisma
/nexgsd-super [paste detailed PRD here]
```
...
IF directory contains existing source files (not just .git or config):
  ...
  │                                                     │
  │  1. git checkout -b nexgsd-super/[feature-name]        │
  ...
  │  branch nexgsd-super/[name]. Your main branch is safe."│
  └─────────────────────────────────────────────────────┘
...
Run the equivalent of `/nexgsd-new-project` autonomously:
...
- Build: ✅ Clean
- Branch: [main or nexgsd-super/feature-name]
- Total time: [duration]
...
```bash
# Review changes:
git diff main...nexgsd-super/[feature-name]

# Merge when ready:
git checkout main
git merge nexgsd-super/[feature-name]
...
Read and follow `.agent/workflows/nexgsd-audit.md` to run a full quality audit.
...
If `.planning/nexgsd-config.json` has `deploy_platform` set:
Read and follow `.agent/workflows/nexgsd-deploy.md` to deploy to production.

---

## Stage 8 — Send Notification

If the user provided an ntfy channel during the interview (Q_ntfy), or if ntfy is configured in nexgsd-config.json:
```bash
