...
If not found:
```
No MertGSD project found in this directory.
Run /mertgsd-new-project to initialize.
```
Stop here.
...
If STATE.md doesn't exist, report "Project exists but STATE.md is missing. Run /mertgsd-new-project to reinitialize."
...
|---------------|------------|
| Phase has no CONTEXT.md | `/mertgsd-discuss N` to capture preferences |
| Phase has context but no plans | `/mertgsd-plan N` to create plans |
| Phase has plans but not executed | `/mertgsd-execute N` to execute |
| Phase executed but not verified | `/mertgsd-verify N` to test |
| Phase verified and passed | `/mertgsd-discuss N+1` or `/mertgsd-plan N+1` for next phase |
| All phases complete | `/mertgsd-new-project` for next milestone |
