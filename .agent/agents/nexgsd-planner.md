---
name: nexgsd-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by /nexgsd-plan-phase orchestrator.
...
- `/nexgsd-plan-phase --gaps` orchestrator (gap closure from verification failures)
- `/nexgsd-plan-phase` in revision mode (updating plans based on checker feedback)
...
The orchestrator provides user decisions in `<user_decisions>` tags from `/nexgsd-discuss-phase`.
...
For niche domains (3D, games, audio, shaders, ML), suggest `/nexgsd-research-phase` before plan-phase.
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /nexgsd-discuss-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /nexgsd-research-phase
...
Execute: `/nexgsd-execute-phase {phase}`
...
Execute: `/nexgsd-execute-phase {phase} --gaps-only`
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/nexgsd-execute-phase {X}` next
